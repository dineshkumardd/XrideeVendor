            ...
            <FlatList
              ...
              renderItem={({item, index}) => (
                <View style={styles.itemView}>
                  ...
                  {item.status === 1 && (
                    <Text style={styles.txtStatus}>Activated</Text>
                  )}

                  {item.status === 2 && (
                    <Text style={styles.txtPending}>Pending</Text>
                  )}
                  {item.status === 3 && (
                    <Text style={styles.txtBlock}>Blocked</Text>
                  )}

                  <TouchableOpacity
                    onPress={() => navigation.push(Routes.VehicleDetails)}>
                    <Text style={styles.review}>View / Edit</Text>
                  </TouchableOpacity>
                </View>
              )}
            />
